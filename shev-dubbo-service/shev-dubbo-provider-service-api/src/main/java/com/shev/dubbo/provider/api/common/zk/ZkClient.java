package com.shev.dubbo.provider.api.common.zk;
...
/*
    ...
    *//**
     * 节点类型:0-持久化节点(默认) 1-临时节点 2-持久顺序节点 3-临时顺序节点
     * @param mode
     * @param path
     *//*
    public void createNode(CreateMode mode, String path) {
        try
        {
            client.create()
                    ...
                    .withMode(mode==null?CreateMode.PERSISTENT:mode)
                    .forPath(path);
        } catch (Exception e)
        {
            logger.error("注册出错", e);
    ...
    public void setNodeData(String path, String datas)
    {
        try
        {
            client.setData().forPath(path, datas.getBytes("UTF-8"));
        }catch (Exception ex)
        {
            logger.error("{}",ex);
        }
